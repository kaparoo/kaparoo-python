...
from kaparoo.filesystem.atomic import AtomicFile, _default_file_mode
...
def test_context_manager_commits_on_success(tmp_path: Path):
    ...
    with AtomicFile(dest, encoding="utf-8") as f:
        assert f.path == dest
        f.write("payload")
        f.flush()
...
def test_context_manager_aborts_on_exception(tmp_path: Path):
    ...
    with pytest.raises(RuntimeError, match="boom"), AtomicFile(dest) as f:  # noqa: PT012
        f.write("half")
        raise boom
...
def test_exception_keeps_existing_file_intact(tmp_path: Path):
    ...
    with pytest.raises(RuntimeError), AtomicFile(dest, overwrite=True) as f:  # noqa: PT012
...
def test_explicit_commit_returns_path(tmp_path: Path):
    dest = tmp_path / "data.txt"
    f = AtomicFile(dest, encoding="utf-8")
    f.write("abc")
...
def test_binary_mode_writes_bytes(tmp_path: Path):
    ...
    with AtomicFile(dest, binary=True) as f:
...
def test_binary_overwrite(tmp_path: Path):
    ...
    with AtomicFile(dest, binary=True, overwrite=True) as f:
        f.write(b"new")
...
def test_overwrite_true_replaces(tmp_path: Path):
    ...
    with AtomicFile(dest, overwrite=True, encoding="utf-8") as f:
        f.write("new")
...
def test_commit_raises_if_destination_appears_after_init(tmp_path: Path):
    ...
    # overwrite is False.
    dest = tmp_path / "data.txt"
    f = AtomicFile(dest, encoding="utf-8")  # dest absent here
    f.write("mine")
...
def test_reexported_from_package():
    ...
    assert filesystem.AtomicFile is AtomicFile
