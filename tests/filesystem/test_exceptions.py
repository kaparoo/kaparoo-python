...
def test_message_and_attributes():
    ...
    assert err.ext == "xyz"
    assert err.expected == ("bin", "txt")
    assert err.kind == ""
    assert str(err) == "unsupported extension 'xyz' (expected one of bin, txt)"


def test_kind_labels_the_message():
    err = UnsupportedExtensionError("xyz", ["bin"], kind="phase")
    assert err.kind == "phase"
    assert str(err) == "unsupported phase extension 'xyz' (expected one of bin)"
...
def test_normalizes_dedups_and_drops_empties():
    ...
    assert err.expected == ("bin", "txt")  # normalized, de-duplicated, "." dropped
